...
[Arch Linux][] is a fantastic GNU/Linux distribution. It features:

- A rolling release development cycle (install once, update forever);
- An excellent package manager, called [pacman][];
- The [ArchWiki][], which is by far my most favourite item in this list;
- The Arch Linux User Repository, or [AUR][] for short, which is a community
  driven repository for (mostly) Open Source Software.
...
28. Generate an fstab with `UUID`'s:
    ...
    ke sure everything is in the right place:
